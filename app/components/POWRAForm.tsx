			...
			const fetchPOWRA = async () => {
				try {
					...
					const maxId = Math.max(
						...data.controlMeasures.map((cm: ControlMeasure) => cm.id),
						0,
					);
	...
	const addControlMeasure = () => {
		setFormData((prev) => ({
			...
			controlMeasures: [
				...
				{ id: nextControlMeasureId, hazardNo: "", measures: "", risk: "L" },
			],
		}));
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.error || `Failed to ${powraId ? "update" : "create"} POWRA`,
				);
			}
			setSuccessMessage(
			...
			setError(
				`Failed to ${powraId ? "update" : "create"} POWRA: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		} finally {
			setIsLoading(false);
		}
		...
		<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div key={field}>
						...
						<Input
							...
							onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
								handleHeaderFieldChange(field, e.target.value)
							}
						/>
					</div>
				))}
					...
					<div key={item} className="flex items-center space-x-2">
						<Checkbox
							...
							onCheckedChange={(checked: boolean) =>
								handleChecklistChange(item, checked)
							}
						/>
			...
			<div>
				...
				<Textarea
					...
					onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
						setFormData((prev) => ({ ...prev, reviewComments: e.target.value }))
					}
				/>
			</div>
